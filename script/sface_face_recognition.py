...
def recognize_face(image, face_detector, face_recognizer, file_name=None):
    ...
    if image.shape[0] > 1000:
        image = cv2.resize(image, (0, 0),
                           fx=EXPECT_INPUT_SIZE / image.shape[0], fy=EXPECT_INPUT_SIZE / image.shape[0])
...
def main():
    ...
    while True:
        ...
        for idx, (face, feature) in enumerate(zip(faces, fetures)):
            ...
            ratio = image.shape[0] / EXPECT_INPUT_SIZE
            box = list(map(int, face[:4] * ratio))
        ...
        end_hand = time.time()
        print(
            f'speed of a loop = {end_hand - start_hand} means {1/(end_hand - start_hand)} frames per second')
